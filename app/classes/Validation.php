							...
							case 'date':
									if($value){
										...
										if(!(preg_match($regex, $name))){
											array_push($this->_errors, "{$name} is not a valid date");
										}
									}
							break;
							case 'email': 
									if($value){
										$regex = '#^\S+@\S+$#';
										if(!preg_match($regex, $name)){
											array_push($this->_errors, "{$name} is not a valid email address");
										}
									}
							break;
							case 'label': 
									if($value){
										$regex = '/^[0-9a-zA-Z ]+$/';
										if(!preg_match($regex, $name)){
											array_push($this->_errors, "{$name} is not a valid {$field}");
										}
									}
							break;
							case 'unique':	
