...
class supplier {
	...
	public function find($id){
		if(isset($id)){
			$data = $this->_db->get('Suppliers', ['id', '=', $id], $this->_user->data()->id);
			if(count($data)){
	...
	public function findNext($supplier_id = null){
		...
		if(count($data)){
			if(!$supplier_id){
				...
				//Select the next after $supplier_id
				for($i = 0; $i < count($data->results()); $i++){
					if($data->results()[$i]->id == $supplier_id){
	...
	public function create($fields){
		if(count($fields) == 10){
			$fields['id'] = null;
			if(!$this->_db->insert('Suppliers', $fields, $this->_user->data()->id)){
